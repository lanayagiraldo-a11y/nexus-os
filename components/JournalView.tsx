    ...
    <div className="flex flex-col h-full overflow-hidden p-6 gap-5">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black" style={{ fontFamily: "var(--font-syne)", color: "#e2e8f0" }}>Journal</h1>
            <p className="text-[12px] capitalize" style={{ fontFamily: "var(--font-outfit)", color: "rgba(148,163,184,0.5)" }}>{today}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {obsidianOk === true && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{ background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.2)" }}>
              ...
              <span className="text-[10px]" style={{ fontFamily: "var(--font-jetbrains)", color: "#34d399" }}>Obsidian sync</span>
            </div>
          )}
          {obsidianOk === false && (
            <div className="px-2.5 py-1 rounded-full" style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.2)" }}>
      ...
      {savedContent && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl p-4 overflow-y-auto max-h-48"
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-semibold tracking-widest uppercase" style={{ fontFamily: "var(--font-jetbrains)", color: "#a78bfa" }}>
      ...
      <div className="flex-1 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className="text-[11px]" style={{ fontFamily: "var(--font-jetbrains)", color: "rgba(148,163,184,0.4)" }}>
            Nueva entrada {voiceState === "listening" ? "🎤 Escuchando…" : ""}
          </span>
          <span className="text-[10px]" style={{ fontFamily: "var(--font-jetbrains)", color: "rgba(148,163,184,0.3)" }}>
            {wordCount} palabras
          </span>
        </div>
        ...
        <textarea value={entry} onChange={e => setEntry(e.target.value)}
          ...
          style={{
            fontFamily: "var(--font-outfit)", color: "#e2e8f0",
            background: "rgba(13,20,40,0.9)",
            border: voiceState === "listening" ? "1px solid rgba(251,113,133,0.4)" : "1px solid rgba(167,139,250,0.2)",
        ...
        <div className="flex items-center gap-3">
          {isSupported && (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={startVoice}
              ...
              style={{
                background: voiceState === "listening" ? "rgba(251,113,133,0.15)" : "rgba(167,139,250,0.08)",
                border: voiceState === "listening" ? "1px solid rgba(251,113,133,0.35)" : "1px solid rgba(167,139,250,0.2)",
                cursor: "pointer",
              }}>
              {voiceState === "listening"
                ? <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 0.8, repeat: Infinity }}><Mic size={14} style={{ color: "#fb7185" }} /></motion.div>
                : <MicOff size={14} style={{ color: "#a78bfa" }} />}
              <span className="text-[12px] font-semibold" style={{ fontFamily: "var(--font-syne)", color: voiceState === "listening" ? "#fb7185" : "#a78bfa" }}>
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={saveJournal}
            ...
            style={{
              background: saved ? "rgba(52,211,153,0.15)" : entry.trim() ? "rgba(167,139,250,0.15)" : "rgba(255,255,255,0.04)",
              border: saved ? "1px solid rgba(52,211,153,0.35)" : entry.trim() ? "1px solid rgba(167,139,250,0.35)" : "1px solid rgba(255,255,255,0.06)",
              cursor: entry.trim() ? "pointer" : "default",
            }}>
            <Save size={14} style={{ color: saved ? "#34d399" : "#a78bfa" }} />
            <span className="text-[12px] font-semibold" style={{ fontFamily: "var(--font-syne)", color: saved ? "#34d399" : "#a78bfa" }}>
