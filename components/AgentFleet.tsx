...
export interface ProviderHealth{id?:string;configured:boolean;reachable:boolean|null;latency:number|null;error?:string;}
...
function StatusBadge({health,accent,accentRgb}:{health:ProviderHealth;accent:string;accentRgb:string}){
  const st=!health.configured?"key":health.reachable===null?"checking":health.reachable?"online":"auth";
  const m={online:{label:"Online",color:"#00A676",pulse:true},auth:{label:"Auth error",color:"#EF4444",pulse:false},key:{label:"Add API Key",color:"#6D28D9",pulse:false},checking:{label:"Checking",color:accent,pulse:true}};
  const c=m[st];
  ...
  return(
    <motion.div initial={{y:28,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:index*0.09,duration:0.55,ease:[0.22,1,0.36,1]}} onClick={isReady?onOpen:undefined} className="relative overflow-hidden rounded-xl flex flex-col" style={{background:"rgba(247,239,226,0.85)",border:`1px solid rgba(${rgb},0.18)`,cursor:isReady?"pointer":"default",minHeight:220}} whileHover={isReady?{y:-4,borderColor:`rgba(${rgb},0.45)`,boxShadow:`0 20px 60px rgba(${rgb},0.15)`,transition:{duration:0.25}}:undefined}>
      ...
      <div className="relative z-10 flex flex-col h-full p-5">
        ...
        {isReady?<motion.div className="flex items-center gap-2" whileHover={{x:3}} transition={{type:"spring",stiffness:400,damping:25}}><span className="text-[12px] font-semibold" style={{fontFamily:"var(--font-syne)",color:accent}}>Open Chat</span><ArrowRight size={13} style={{color:accent}}/></motion.div>:<div className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"#991B1B"}}>{!health.configured?`Set ${provider.envKey} in .env.local`:id==="claude"?"Actualizar CLAUDE_API_KEY en Netlify":id==="hermes"?"Start: hermes serve --port 8080":"Unreachable"}</div>}
      </div>
    </motion.div>
  );
}
