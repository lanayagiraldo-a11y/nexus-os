...
const MESH:Record<ProviderId,string>={claude:"radial-gradient(ellipse at 20% 20%,rgba(167,139,250,0.12) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(109,40,217,0.08) 0%,transparent 60%)",openai:"radial-gradient(ellipse at 20% 20%,rgba(52,211,153,0.1) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(16,163,127,0.07) 0%,transparent 60%)",gemini:"radial-gradient(ellipse at 20% 20%,rgba(96,165,250,0.1) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(34,211,238,0.07) 0%,transparent 60%)",hermes:"radial-gradient(ellipse at 20% 20%,rgba(251,191,36,0.1) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(245,158,11,0.07) 0%,transparent 60%)"};
...
function StatusBadge({health,accent,accentRgb}:{health:ProviderHealth;accent:string;accentRgb:string}){
  ...
  const m={online:{label:"Online",color:"#34d399",pulse:true},offline:{label:"Offline",color:"#fb7185",pulse:false},key:{label:"Add API Key",color:"#fbbf24",pulse:false},checking:{label:"Checking",color:accent,pulse:true}};
  const c=m[st];
    ...
    <motion.div initial={{y:28,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:index*0.09,duration:0.55,ease:[0.22,1,0.36,1]}} onClick={isReady?onOpen:undefined} className="relative overflow-hidden rounded-xl flex flex-col" style={{background:"rgba(8,14,31,0.85)",border:`1px solid rgba(${rgb},0.18)`,cursor:isReady?"pointer":"default",minHeight:220}} whileHover={isReady?{y:-4,borderColor:`rgba(${rgb},0.45)`,boxShadow:`0 20px 60px rgba(${rgb},0.15)`,transition:{duration:0.25}}:undefined}>
      ...
      <div className="relative z-10 flex flex-col h-full p-5">
        ...
        <div className="mb-3">
          ...
          <p className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"rgba(148,163,184,0.5)"}}>{provider.provider} · {provider.fullName}</p>
        </div>
        ...
        <div className="flex items-center gap-4 pb-4 mb-4" style={{borderBottom:`1px solid rgba(${rgb},0.1)`}}>
          <div className="flex items-center gap-1.5"><MessageSquare size={11} style={{color:`rgba(${rgb},0.55)`}}/><span className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"rgba(148,163,184,0.6)"}}>{msgCount} msgs</span></div>
          <div className="flex items-center gap-1.5"><Clock size={11} style={{color:`rgba(${rgb},0.55)`}}/><span className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"rgba(148,163,184,0.6)"}}>{health.latency?`${health.latency}ms`:"—"}</span></div>
        ...
        {isReady?<motion.div className="flex items-center gap-2" whileHover={{x:3}} transition={{type:"spring",stiffness:400,damping:25}}><span className="text-[12px] font-semibold" style={{fontFamily:"var(--font-syne)",color:accent}}>Open Chat</span><ArrowRight size={13} style={{color:accent}}/></motion.div>:<div className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"rgba(148,163,184,0.3)"}}>{!health.configured?`Set ${provider.envKey} in .env.local`:id==="hermes"?"Start: hermes serve --port 8080":"Unreachable"}</div>}
      </div>
    </motion.div>
  );
}
    ...
    <div className="flex flex-col gap-6 p-6 overflow-y-auto h-full">
      <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} transition={{duration:0.4}}>
        <div className="flex items-end justify-between">
          <div><h1 className="text-2xl font-black tracking-tight mb-1" style={{fontFamily:"var(--font-syne)",color:"#e2e8f0"}}>AI Fleet</h1><p className="text-[13px]" style={{fontFamily:"var(--font-outfit)",color:"rgba(148,163,184,0.5)"}}>{onlineCount} of {PROVIDERS.length} agents online · click to chat</p></div>
          <motion.button onClick={check} whileHover={{scale:1.05}} whileTap={{scale:0.95}} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg" style={{background:"rgba(34,211,238,0.07)",border:"1px solid rgba(34,211,238,0.15)",cursor:"pointer"}}>
            <motion.div animate={checking?{rotate:360}:{}} transition={{duration:0.7,repeat:checking?Infinity:0,ease:"linear"}}><RefreshCw size={11} style={{color:"#22d3ee"}}/></motion.div>
            <span className="text-[10px]" style={{fontFamily:"var(--font-jetbrains)",color:"#22d3ee"}}>{lastChecked?`${lastChecked}`:"Refresh"}</span>
          </motion.button>
        </div>
      </motion.div>
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.45}} className="rounded-xl p-4" style={{background:"rgba(34,211,238,0.03)",border:"1px solid rgba(34,211,238,0.08)"}}>
        <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-2.5" style={{fontFamily:"var(--font-syne)",color:"rgba(34,211,238,0.5)"}}>Setup · .env.local</p>
        <div className="grid grid-cols-2 gap-x-8 gap-y-1">
          {[{key:"ANTHROPIC_API_KEY",val:"sk-ant-...",color:"#a78bfa"},{key:"OPENAI_API_KEY",val:"sk-proj-...",color:"#34d399"},{key:"GEMINI_API_KEY",val:"AIza...",color:"#60a5fa"},{key:"HERMES_BASE_URL",val:"http://localhost:11434",color:"#fbbf24"}].map(({key,val,color})=>(
            <div key={key} className="text-[11px]" style={{fontFamily:"var(--font-jetbrains)",color:"rgba(148,163,184,0.45)"}}><span style={{color}}>{key}</span>=<span>{val}</span></div>
          ))}
        </div>
      </motion.div>
    </div>
