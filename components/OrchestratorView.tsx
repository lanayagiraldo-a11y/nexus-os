...
function MarkdownLike({ text }: { text: string }) {
  return (
    <div className="whitespace-pre-wrap text-sm leading-relaxed" style={{ fontFamily: "var(--font-outfit)", color: "rgba(226,232,240,0.86)" }}>
      {text}
    </div>
  );
}
  ...
  return (
    <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-4"
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        {result.status === "completed" ? <CheckCircle2 size={16} color="#00A676" /> : <AlertTriangle size={16} color="#EF4444" />}
      </div>
      {result.status === "completed" ? <MarkdownLike text={result.content} /> : <div className="text-sm" style={{ color: "#EF4444", fontFamily: "var(--font-outfit)" }}>Error: {result.error}</div>}
    </motion.div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto overscroll-contain">
      <div className="p-4 sm:p-5 lg:p-6 pb-24 md:pb-6 flex flex-col gap-5">
        <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ background: "rgba(109,40,217,0.1)", border: "1px solid rgba(109,40,217,0.22)" }}>
                <BrainCircuit size={20} color="#6D28D9" />
              </div>
              <div>
          ...
          <div className="px-3 py-1.5 rounded-full self-start" style={{ background: "rgba(109,40,217,0.08)", border: "1px solid rgba(109,40,217,0.22)", fontFamily: "var(--font-jetbrains)", color: "#6D28D9", fontSize: 11 }}>Ejecuta solo guardar/pendientes; envíos requieren aprobación</div>
        </motion.div>

        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-4">
          <div className="rounded-2xl p-4" style={{ background: "rgba(247,239,226,0.9)", border: "1px solid rgba(109,40,217,0.13)", backdropFilter: "blur(16px)" }}>
            <div className="flex items-center gap-2 mb-3">
              <Database size={16} color="#6D28D9" />
              <h2 className="text-sm font-black" style={{ fontFamily: "var(--font-syne)", color: "#6D28D9" }}>Selector de contexto</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
              {CONTEXT_OPTIONS.map(option => {
                const Icon = option.icon;
                const active = contextType === option.type && contextLabel === option.label;
                return <button key={`${option.type}-${option.label}`} onClick={() => applyContextOption(option)} className="p-3 rounded-xl text-left" style={{ background: active ? "rgba(109,40,217,0.1)" : "rgba(7,24,46,0.03)", border: active ? "1px solid rgba(109,40,217,0.32)" : "1px solid rgba(7,24,46,0.07)", cursor: "pointer" }}>
                  <div className="flex items-center gap-2"><Icon size={14} color={active ? "#6D28D9" : "rgba(31,41,55,0.65)"}/><span className="text-[12px] font-bold" style={{ fontFamily: "var(--font-syne)", color: active ? "#6D28D9" : "#cbd5e1" }}>{option.label}</span></div>
          ...
          <div className="rounded-2xl p-4" style={{ background: "rgba(247,239,226,0.9)", border: "1px solid rgba(109,40,217,0.14)", backdropFilter: "blur(16px)" }}>
            <div className="flex items-center gap-2 mb-3">
              <Users size={16} color="#6D28D9" />
              <h2 className="text-sm font-black" style={{ fontFamily: "var(--font-syne)", color: "#6D28D9" }}>Agentes convocados</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2 gap-2">
              {PROVIDERS.map(provider => {
                ...
                return <button key={provider.id} onClick={() => toggleAgent(provider.id)} className="flex items-center gap-2 rounded-xl p-2.5 text-left" style={{ background: active ? `rgba(${rgb},0.10)` : "rgba(7,24,46,0.03)", border: active ? `1px solid rgba(${rgb},0.30)` : "1px solid rgba(7,24,46,0.07)", cursor: "pointer" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-black" style={{ fontFamily: "var(--font-syne)", color: active ? ACCENT[provider.id] : "#cbd5e1" }}>{provider.name}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          {MODES.map(m => {
            ...
            return <motion.button key={m.id} onClick={() => setMode(m.id)} whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} className="p-3 rounded-xl text-left"
              style={{ background: active ? "rgba(109,40,217,0.1)" : "rgba(247,239,226,0.7)", border: active ? "1px solid rgba(109,40,217,0.35)" : "1px solid rgba(7,24,46,0.07)", cursor: "pointer" }}>
              <div className="flex items-center gap-2 mb-1"><Icon size={14} color={active ? "#6D28D9" : "rgba(31,41,55,0.65)"}/><span className="text-[12px] font-bold" style={{ fontFamily: "var(--font-syne)", color: active ? "#6D28D9" : "#cbd5e1" }}>{m.label}</span></div>
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(247,239,226,0.9)", border: "1px solid rgba(109,40,217,0.13)", backdropFilter: "blur(16px)" }}>
          ...
          <div className="flex justify-stretch sm:justify-end mt-4">
            <motion.button onClick={run} disabled={!mission.trim() || running} whileHover={mission.trim() && !running ? { scale: 1.02 } : undefined} whileTap={mission.trim() && !running ? { scale: 0.98 } : undefined} className="flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-2.5 rounded-xl font-bold"
              style={{ background: mission.trim() && !running ? "#6D28D9" : "rgba(7,24,46,0.07)", color: mission.trim() && !running ? "#F7EFE2" : "rgba(31,41,55,0.35)", fontFamily: "var(--font-syne)", cursor: mission.trim() && !running ? "pointer" : "default" }}>
        ...
        {error && <div className="rounded-xl p-3 text-sm" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.24)", color: "#EF4444", fontFamily: "var(--font-outfit)" }}>Error: {error}</div>}

        <AnimatePresence>
          {running && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-2xl p-5 text-center" style={{ background: "rgba(109,40,217,0.05)", border: "1px solid rgba(109,40,217,0.14)", color: "#6D28D9", fontFamily: "var(--font-syne)" }}>El consejo de agentes está trabajando con el contexto seleccionado…</motion.div>}
        </AnimatePresence>

        {data && <div className="flex flex-col gap-5">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-5" style={{ background: "linear-gradient(135deg, rgba(109,40,217,0.08), rgba(109,40,217,0.06))", border: "1px solid rgba(109,40,217,0.2)" }}>
            <div className="flex items-center gap-2 mb-3"><Lightbulb size={18} color="#6D28D9"/><h2 className="text-lg font-black" style={{ fontFamily: "var(--font-syne)", color: "#6D28D9" }}>Síntesis de Hermi</h2></div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-5" style={{ background: "rgba(247,239,226,0.92)", border: "1px solid rgba(0,166,118,0.18)", backdropFilter: "blur(16px)" }}>
            <div className="flex items-center gap-2 mb-3"><ListChecks size={18} color="#00A676"/><h2 className="text-lg font-black" style={{ fontFamily: "var(--font-syne)", color: "#00A676" }}>Panel de ejecución</h2></div>
            ...
            {extractedTasks.length > 0 && <div className="mb-4 rounded-xl p-3" style={{ background: "rgba(0,166,118,0.05)", border: "1px solid rgba(0,166,118,0.14)" }}>
              <div className="text-[11px] font-bold mb-2" style={{ fontFamily: "var(--font-syne)", color: "#00A676" }}>Pendientes detectados</div>
              <div className="flex flex-col gap-1.5">
                {extractedTasks.map(task => <div key={task} className="text-[12px]" style={{ fontFamily: "var(--font-outfit)", color: "rgba(226,232,240,0.78)" }}>- [ ] {task}</div>)}
              </div>
            </div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2">
              <button onClick={() => executeSynthesis("save-synthesis")} disabled={!!executing} className="flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-[12px] font-bold" style={{ background: "rgba(109,40,217,0.10)", border: "1px solid rgba(109,40,217,0.24)", color: "#6D28D9", fontFamily: "var(--font-syne)", cursor: executing ? "default" : "pointer" }}><Save size={14}/>Guardar síntesis</button>
              <button onClick={() => executeSynthesis("create-tasks")} disabled={!!executing || extractedTasks.length === 0} className="flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-[12px] font-bold" style={{ background: extractedTasks.length ? "rgba(0,166,118,0.10)" : "rgba(7,24,46,0.04)", border: extractedTasks.length ? "1px solid rgba(0,166,118,0.24)" : "1px solid rgba(7,24,46,0.07)", color: extractedTasks.length ? "#00A676" : "rgba(31,41,55,0.35)", fontFamily: "var(--font-syne)", cursor: executing || extractedTasks.length === 0 ? "default" : "pointer" }}><ListChecks size={14}/>Crear pendientes</button>
            ...
            {executionStatus && <div className="mt-3 rounded-xl p-3 text-[12px]" style={{ background: executionStatus.startsWith("Error") ? "rgba(239,68,68,0.08)" : "rgba(0,166,118,0.08)", border: executionStatus.startsWith("Error") ? "1px solid rgba(239,68,68,0.22)" : "1px solid rgba(0,166,118,0.22)", color: executionStatus.startsWith("Error") ? "#EF4444" : "#00A676", fontFamily: "var(--font-outfit)" }}>{executionStatus}</div>}
          </motion.div>
