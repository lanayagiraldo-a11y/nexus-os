...
function ClaudeIcon({size}:{size:number}) {
  return <svg width={size*0.55} height={size*0.55} viewBox="0 0 24 24" fill="none">
    <path d="M12 3 C6.48 3 2 7.48 2 13 C2 16.1 3.4 18.9 5.6 20.8 L5.6 21 L18.4 21 L18.4 20.8 C20.6 18.9 22 16.1 22 13 C22 7.48 17.52 3 12 3Z" fill="none" stroke="rgba(167,139,250,0.9)" strokeWidth="1.5" strokeLinejoin="round"/>
    <path d="M8 13.5 C8 11.02 9.79 9 12 9 C14.21 9 16 11.02 16 13.5" stroke="rgba(167,139,250,1)" strokeWidth="1.8" strokeLinecap="round" fill="none"/>
    <circle cx="12" cy="14.5" r="1.5" fill="#a78bfa"/>
  </svg>;
}
...
function GeminiIcon({size}:{size:number}) {
  ...
  return <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
    ...
    <defs><linearGradient id="gg" x1="2" y1="2" x2="22" y2="24" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#60A5FA"/><stop offset="50%" stopColor="#818CF8"/><stop offset="100%" stopColor="#22d3ee"/></linearGradient></defs>
  </svg>;
}
function HermesIcon({size}:{size:number}) {
  const s=size*0.52;
  return <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
    <line x1="12" y1="3" x2="12" y2="21" stroke="rgba(251,191,36,0.9)" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M12 8C10 6 6 6 5 8C6 9 9 8.5 12 10" fill="rgba(251,191,36,0.7)"/>
    <path d="M12 8C14 6 18 6 19 8C18 9 15 8.5 12 10" fill="rgba(251,191,36,0.7)"/>
    <path d="M12 12C10.5 11 8 11 7 12.5C8 13 10 12.5 12 13.5" fill="rgba(251,191,36,0.5)"/>
    <path d="M12 12C13.5 11 16 11 17 12.5C16 13 14 12.5 12 13.5" fill="rgba(251,191,36,0.5)"/>
    <circle cx="12" cy="6" r="2" fill="#fbbf24" opacity="0.9"/>
  </svg>;
}
const CFG: Record<ProviderId,{bg:string;border:string;shadow:string;Icon:React.ComponentType<{size:number}>}> = {
  claude:  {bg:"radial-gradient(circle at 35% 35%, rgba(167,139,250,0.25), rgba(88,28,135,0.1))",  border:"rgba(167,139,250,0.35)", shadow:"0 0 24px rgba(167,139,250,0.3),0 0 48px rgba(167,139,250,0.1)", Icon:ClaudeIcon},
  openai:  {bg:"radial-gradient(circle at 35% 35%, rgba(52,211,153,0.2), rgba(5,150,105,0.08))",   border:"rgba(52,211,153,0.35)",  shadow:"0 0 24px rgba(52,211,153,0.3),0 0 48px rgba(52,211,153,0.1)",  Icon:OpenAIIcon},
  gemini:  {bg:"radial-gradient(circle at 35% 35%, rgba(96,165,250,0.2), rgba(6,182,212,0.08))",   border:"rgba(96,165,250,0.35)",  shadow:"0 0 24px rgba(96,165,250,0.3),0 0 48px rgba(96,165,250,0.1)",  Icon:GeminiIcon},
  hermes:  {bg:"radial-gradient(circle at 35% 35%, rgba(251,191,36,0.2), rgba(217,119,6,0.08))",   border:"rgba(251,191,36,0.35)",  shadow:"0 0 24px rgba(251,191,36,0.3),0 0 48px rgba(251,191,36,0.1)",  Icon:HermesIcon},
};
